...
use tokio_rustls::rustls::{ServerConfig, pki_types::CertificateDer, pki_types::PrivatePkcs8KeyDer};
...
impl TlsSpoofingProxy {
    pub async fn start(impersonate_client: Client) -> Result<Self, Error> {
        ...
        Ok(Self { port, shutdown_tx: Some(shutdown_tx) })
    }
    ...
    async fn handle_request(
        ...
        if Method::CONNECT == req.method() {
            ...
            let mut req_builder = client.request(rquest::Method::from_bytes(method.as_str().as_bytes()).unwrap(), hyper_uri);
            ...
            let response = match req_builder.send().await {
                Ok(resp) => resp,
                Err(_) => return Ok(Response::builder().status(502).body(rquest::Body::from("")).unwrap()),
            };
    ...
    async fn handle_tunnel(upgraded: Upgraded, target_host: String, client: Arc<Client>) -> Result<(), Error> {
        ...
        }).await.map_err(|e| Error::Internal(anyhow::anyhow!("Join error: {}", e)))?
        ...
        if let Err(err) = http1::Builder::new()
            .preserve_header_case(true)
            .title_case_headers(true)
            .serve_connection(tls_io, service_fn(move |inner_req| {
                let host = target_host.clone();
                let client_ref = Arc::clone(&client);
                async move {
                    Self::forward_tls_request(inner_req, host, client_ref).await
                }
            }))
            .await
        {
    ...
    async fn forward_tls_request(
        ...
        let uri = format!("https://{}{}", host, req.uri().path_and_query().map(|x| x.as_str()).unwrap_or("/"));
        ...
        match req_builder.send().await {
            ...
            Err(_) => Ok(Response::builder().status(502).body(rquest::Body::from("")).unwrap()),
        }
    }
}
