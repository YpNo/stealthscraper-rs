...
async fn test_scraper_end_to_end_navigation() {
    ...
    let tab = scraper.new_stealth_tab().expect("Failed to create stealth tab");

    // 4. Navigate to a simple, highly-available website
    tab.navigate_to("https://nowsecure.nl").expect("Failed to navigate");
    ...
    let header_element = tab.wait_for_element("title").expect("Failed to find <title> element on nowsecure.nl");
    let text = header_element.get_inner_text().expect("Failed to get inner text");
