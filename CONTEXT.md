...
4.  **Impersonation Client** (`wreq` + `emulation` + `cert_compression`): the proxy re-dispatches requests through `wreq`, shaped by a first-party emulation table. The upstream client is hot-swappable, so the egress proxy rotates without relaunching the browser. The same emulation is reachable without any of the above: `impersonation_client(&profile)` (`client`) returns a `wreq::ClientBuilder` in the **default build**, for targets that gate on the fingerprint and serve no JS challenge. `cert_compression` supplies the RFC 8879 brotli/zlib codecs, which `wreq` 6 requires the caller to provide and which are part of the `ClientHello`.
...
- **Dependency pins worth knowing**: `wreq` is `6.0.0-rc.31` — a release candidate, accepted because the whole 5.x line is yanked (see the note in `README.md`), and re-exported, so it is public API. `brotli` is held at 8 although 9 exists, so the codec is shared with the copy `wreq` already pulls rather than duplicated.
